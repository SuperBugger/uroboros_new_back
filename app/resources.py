...
class BaseExportResource:
    def export_data(self, data, export_format, filename, headers=None):
        """Обрабатывает экспорт данных в указанный формат."""
        if export_format == 'csv':
            return self.export_to_csv(data, filename, headers)
        elif export_format == 'excel':
            return self.export_to_excel(data, filename, headers)
        elif export_format == 'pdf':
            return self.export_to_pdf(data, filename, headers)
        elif export_format == 'print':
            return self.export_to_html(data, headers)
    ...
    def export_to_pdf(self, data, filename, headers):
        """Экспорт в PDF."""
        output = io.BytesIO()
        pdf = canvas.Canvas(output, pagesize=A4)
        pdf.setFont("Helvetica", 10)
    ...
    def export_to_html(self, data, headers):
